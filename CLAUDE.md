...
| `sysml-reviewer`      | Before committing SysML v2 model changes — runs the validator, then checks layout, naming, units, value provenance, requirement structure, satisfy/verify traceability, `@RosTopic` mapping. |
| `mbse-architect`      | Before modeling — system decomposition, where ports/interfaces go, requirement tree, which analysis proves what, SysML vs Simulink split. |
...
### MBSE — SysML v2 / MATLAB / Simulink

| Skill                    | Topic |
|--------------------------|-------|
| `sysml_v2_modeling`      | Write/extend/validate/render SysML v2 text models (structure, ports/flows, requirements, calcs, analysis & verification cases), derived from the ROS 2 workspace |
| `system_composer_sysml`  | SysML v2 ↔ MATLAB/Simulink/System Composer via the MATLAB MCP + Simulink Agentic Toolkit — architecture build, SysML export, numeric analysis, Requirements Toolbox links |
...
| `sysml_v2.md`           | SysML v2 textual notation — def/usage table, verified idioms, project modeling conventions, ROS 2 ↔ SysML mapping, pitfalls |
| `matlab_simulink_mbse.md` | MATLAB MCP Server + Simulink Agentic Toolkit, release capability matrix (SysML v2 = R2026a+), WSL setup, round-trip pipelines, SysML ↔ System Composer mapping |
...
* **SysML models never invent numbers** — every value cites its source
  file; unknowns are `TBD`, guesses are `ASSUMPTION`. Validate with
  `/sysml-validate` before handing off.
